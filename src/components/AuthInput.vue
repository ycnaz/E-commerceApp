...
<template>
    <div class="relative">
        <div @click="focusInput" class="group peer flex justify-center items-end w-96 h-12 border bg-gray-100 border-black rounded cursor-text" :class="{'border-rose-500': isValid === false, 'border-emerald-500': isValid}">
            <input @input="validateInput" :type="inputType" v-model="model" :autocomplete="props.autoComplete" :minlength="props.minLength" :maxlength="props.maxLength" ref="inp" class="group-focus: border-none h-7 w-[365px] p-0 focus:ring-0 bg-gray-100 placeholder:text-transparent" :id="props.inputId" :placeholder="props.labelText" required>
        </div>
